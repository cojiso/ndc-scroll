...
<script lang="ts">
  ...
  function handleSelectItem(event: CustomEvent<{ item: NDC9Item, scrollTo: boolean }>) {
    console.log('Layout: handleSelectIte called', event.detail);
...
<div class="layout" class:mobile={isMobile} on:touchstart={handleTouchStart} on:touchmove={handleTouchMove} on:touchend={handleTouchEnd}>
